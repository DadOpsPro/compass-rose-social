...
**Flow:** draft movie JSON in this repo → render silent MP4 via JSON2Video → drop MP4 in Drive → set Sheet status `needs_audio` for Kristin → she adds audio / approves → schedule (later: n8n → Meta/TikTok). Never commit MP4s here.
