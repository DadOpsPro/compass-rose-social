...
- **Fonts** — one type system per Reel. Set `"font"` once at the timeline root (`montserrat` default, or `bebas` / `oswald` / `playfair` / `poppins` / `anton`). Do not swap faces mid-Reel. Different Reels may use different fonts.
